...
#endif
#if LAB_NO>4
solution SD(matrix x0, double h0, double epsilon, int Nmax, matrix* ud, matrix* ad) //najmniejszy spadek
{
	int n = get_len(x0);
	solution X, X1; //X poczatek iteracji, X1 koniec iteracji
	X.x = x0;
	matrix d(n, 1), * P = new matrix[2]; //p[0] = xi, p[1] = di
	solution h; //optymalna alfa 
	double* ab; //poczatkowy przedzial zlotego podzialu
	while (true)
	{
		X.grad();
		d = -X.g; // X.g - dostep do gradientu
		if (h0 < 0)
		{
			P[0] = X.x;
			P[1] = d;
			ab = expansion(0.0, 1, 1.2, Nmax, ud, P);
...
{
	double alfa = (sqrt(5) - 1) / 2; //golden ratio?
	solution A, B, C, D;
	A.x = a;
	B.x = b;
...
solution Powell(matrix x0, double epsilon, int Nmax, matrix *ud, matrix *ad)
{
	...
	while (true)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			A[0] = P.x;
			A[1] = D[i];
			ab = expansion(0.0, 1, 1.2, Nmax, ud, A); //ud - przekazywanie wagi
...
{	//N - liczbaw wymiarów, limits - zakres rozwiazan poczatkowych, mi - wielkosc pop pocza, lambda - wielkosc pop potomnej,
